...
#define LEVEL2_HEIGHT 40

#define LEVEL2_ENEMY_COUNT 10
#define LEVEL2_BULLET_COUNT 10
...
void Level2::Initialize() {
    ...
    state.map = new Map(LEVEL2_WIDTH, LEVEL2_HEIGHT, level2_data, mapTextureID, 1.0f, 7, 4);
    ...
    GLuint enemy2TextureID = Util::LoadTexture("8-bit-enemies.png");

    for (int i = 0; i < LEVEL2_ENEMY_COUNT; i++) {
        state.enemies[i].entityType = ENEMY;
        state.enemies[i].textureID = enemy2TextureID;
    ...
    state.bullets = new Entity[LEVEL2_BULLET_COUNT];
    state.bulletCnt = LEVEL2_BULLET_COUNT;
    GLuint bulletTextureID = Util::LoadTexture("blast.png");
    for (int i = 0; i < LEVEL2_BULLET_COUNT; i++) {
...
void Level2::Update(float deltaTime) {
    ...
    for (int i = 0; i < LEVEL2_BULLET_COUNT; i++) {
        state.bullets[i].Update(deltaTime, state.player, state.enemies, LEVEL2_ENEMY_COUNT, state.map);
    }
...
void Level2::Render(ShaderProgram* program) {
    ...
    Util::DrawText(program, state.fontTextureID, "Level 2", 1, -0.6f, glm::vec3(0.0, 7.5 + state.player->position.y, 0));
